...
proc linear*(image: Image, fromAngle: int, step: float, rotations: int, columns: int, rows: int, bounds: IVec2, logger: Logger): Image =
  let w = bounds.x
  let h = bounds.y
  ...
  for i in countup(0, rotations - 1):
    ...
    result.draw(image, transform, blendMode = NormalBlend, DrawMethod.linear, mask = some(IRect(x: column * w, y: row * h, w: w, h: h)))
